...
├── tooling/
├── deploy/
│   └── worker/                     ← 自有服务器 Docker Compose 部署入口
├── .github/workflows/
├── turbo.json
├── pnpm-workspace.yaml
└── package.json
...
|-------|------|------|---------|
| **用户** | PM + 验收 | — | — |
| **1 基建** | monorepo + CI/CD | 配置 | `turbo.json` `.github/` `tooling/` |
...
| **4 AI** | AI 能力 + 后台作业 | TypeScript | `packages/ai` `apps/worker` |
...
Web 和数据服务继续使用托管平台，后台 Worker 使用用户已有服务器：

| 组件 | 服务 | 费用 |
|------|------|------|
| Web（Next.js） | Vercel | 免费 |
| Worker（Node.js 常驻） | 自有服务器 Docker Compose | 使用已有服务器资源 |
| PostgreSQL | Neon | 免费（0.5GB） |
| Redis | Upstash | 免费（10k 命令/天） |
| 文件存储 | Cloudflare R2 | 免费（10GB） |
| 邮件 | Resend | 免费（3000 封/月） |

Worker 镜像在 Mac 构建为 `linux/amd64` 后导入服务器，生产入口为 `deploy/worker/compose.yml`。运行容器有独立的内存、CPU 和 PID 上限，只建立到 Neon、Upstash、Vercel 和 AI API 的出站连接，不开放公网端口。Railway 仍可作为替代托管方案，但不是当前部署事实。
