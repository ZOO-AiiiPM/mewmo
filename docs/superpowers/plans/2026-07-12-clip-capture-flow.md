...
- [ ] Write failing equivalence tests for fragments, tracking parameters, case, default ports, protocol, and trailing slashes.
- [ ] Run tests and confirm failure.
- [ ] Implement the normalizer and nullable unique Clip identity fields.
- [ ] Generate Prisma Client and run tests until green.
...
- [ ] Write failing tests for stable retryable Clip jobs, no synchronous extraction in create, and duplicate response metadata.
- [ ] Run focused tests and confirm failure.
- [ ] Implement persist-first creation, unique-conflict recovery, and queue-failure status.
...
- [ ] Write failing worker and route contract tests for authorization, status transitions, extraction update, summary enqueue, and retry errors.
- [ ] Run tests and confirm failure.
- [ ] Implement the worker and secured background refresh while preserving user-triggered refresh.
...
- [ ] Add failing tests for awaited submission, disabled duplicate input, truthful toast ownership, existing-record selection, and active-fetch polling.
- [ ] Run focused tests and confirm failure.
- [ ] Implement the async callback contract and cache/list update behavior in both Clip routes.
- [ ] Run focused tests until green.

### Task 5: Batch verification and Linear handoff

- [ ] Run lint, unit, theme, build, and isolated API integration suites.
- [ ] Browser-test slow create, duplicate create, immediate list insertion, background status, retry, and both themes.
- [ ] Record evidence on ZOO-8 and ZOO-11 while leaving them In Progress.
