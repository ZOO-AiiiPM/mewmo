...
test("一级侧边栏入口共用分组标题的 icon 和 label 对齐列", () => {
  ...
  assert.match(sidebar, /mewmo-nav-row__icon--primary" data-icon=\{icon\}/);
  assert.match(sidebar, /data-icon=\{primary \? icon : undefined\}/);
  assert.match(css, /\.mewmo-nav-row:not\(\.mewmo-nav-row--active\) \.mewmo-nav-row__icon--primary\[data-icon="cat"\] \.mewmo-prototype-icon,/);
  assert.match(css, /\.mewmo-nav-row:not\(\.mewmo-nav-row--active\) \.mewmo-nav-row__icon--primary\[data-icon="rss"\] \.mewmo-prototype-icon,/);
  assert.match(css, /\.mewmo-nav-row--active \.mewmo-nav-row__icon--primary\[data-icon="trash"\] \.mewmo-prototype-icon \{\s*transform: scaleX\(1\.1667\);/);
  assert.doesNotMatch(icons, /opticalAlign|SIDEBAR_PRIMARY_ICON_LEFT_INSET/);
});
