import { extractClipBodyHtml, stripHtml } from "./clip-content";
...
export async function fetchClipFromUrl(url: string): Promise<ExtractedClip> {
  ...
  if (!response.ok) throw new Error(`Failed to fetch clip: ${response.status}`);
  ...
  return extractClipFromHtml(html, response.url || url);
}

export function extractClipFromHtml(html: string, pageUrl: string): ExtractedClip {
  const content = extractClipBodyHtml(html);
  const title =
  ...
  const summary =
    rawSummary !== undefined ? normalizeSummary(rawSummary) : summarize(content);
  const favicon = readFavicon(html, pageUrl);
  const coverImage = readCoverImage(html, pageUrl, content);
  const excerpt = summarize(content);
  const sourceName =
...
function normalizeSummary(value: string | undefined): string | undefined {
  ...
  const genericSummaries = new Set(["详尽文档"]);
  return genericSummaries.has(summary) ? undefined : summary;
}
