...
在 `.env.worker` 填入 Neon、Upstash、Vercel 和 AI 配置。`FEED_CRON_SECRET` 必须和 Vercel Production 中的同名变量完全一致。SSH 密码不属于 Worker 配置，不能写入这个文件。
