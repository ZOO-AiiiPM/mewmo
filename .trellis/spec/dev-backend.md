...
- **环境变量由各部署平台 dashboard 注入**（Vercel / Neon / Resend 等），本地调试可选 `.env.local` 覆盖；数据库 URL、API key、OAuth secret 等禁止硬编码。`packages/shared/src/env.ts` 用 Zod 校验各运行入口需要的 env，启动时缺 env 直接报错而非运行时才炸。
