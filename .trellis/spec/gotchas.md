...
> 本文件由 `AGENTS.md` 抽出的项目专属层。模型有强先验，这些是最容易踩错的地方。
...
`.qoder/repowiki/` 是 Qoder 按提交自动生成并导出的代码导航，适合在陌生模块、跨模块改动或架构解释前按关键词检索相关页面和源码入口；它不是需求、API 契约或完成度的真相源。Wiki 会把代码结构扩写成通用设计，且可能引用已移动或不存在的文件，所以其中的字段、命令、能力与行为结论必须回到当前源码、测试、正式 spec 或 `AGENTS.md` 核验，冲突时以后者为准。
