...
- `PendingMutation.payloadJSON` is an opaque canonical ZOO-89 mutation object, not only the nested `data` value. Persisted `mutationId`, `entityKind`, and `op` are integrity metadata; `expectedVersion` is validated only when the optional wire field is present. Invalid rows must remain queued.
