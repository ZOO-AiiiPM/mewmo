...
- **logout 顺序 bug（ZOO-110）**：同时携有效 refresh + 有效 bearer 时，若先吊销 refresh（置 `revoked_at`）再去 `resolveAccessToken(bearer)`，后者因「会话已注销」返回 null → 已成功注销却回 401。修复：让 `revokeByRefreshToken` 返回「是否真的定位并吊销了会话」，路由中**有效 refresh 一旦吊销即权威 204**（不再走 bearer 判失败）；无有效 refresh 时 bearer 才是唯一身份，无效 bearer 仍 401（不可绕过）。写真实行为路由测试（mock 服务模块）确认「valid refresh+bearer → 204」与「invalid bearer 不可绕过」。
