...
- A block menu's selection is optimized for style conversion, not deletion: code and text targets use `TextSelection`, while tables use `NodeSelection`. Structural actions must capture their own target range when the menu opens. Destructive CSS must also match the base menu selector's specificity, and adding a tenth row requires verifying the actual scroll-container height instead of only the outer popup position.
