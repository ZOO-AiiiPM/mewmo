import { fetchFeedDocument, normalizeFeedEntryContent, type ParsedFeedEntry } from "@mewmo/content";
...
export async function fetchInitialFeed(
  ...
  const fetchFeed = dependencies.fetchFeed ?? ((url: string) => fetchFeedDocument(url, {
    ...(dependencies.allowedPrivateOrigins ? { allowedPrivateOrigins: dependencies.allowedPrivateOrigins } : {}),
  }));
