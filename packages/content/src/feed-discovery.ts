...
import { parseFeedDocument } from "./feed";
import { fetchOutbound } from "./outbound";
...
export async function discoverFeeds(query: string, deps: DiscoverDeps = {}): Promise<DiscoveredFeed[]> {
  ...
  const fetchFeed = deps.fetchFeed ?? safeFetch;
...
const safeFetch: typeof fetch = (input, init) => fetchOutbound(
